...
export function Portfolio() {
  return <section id="portfolio" className={`${s.section} ${s.portfolio}`} aria-labelledby="portfolio-title">
    ...
    <div className={s.portfolioIntro}><div><h2 id="portfolio-title" className={s.display} data-reveal>My<br /><em>portfolio.</em></h2><p className={s.serifNote}>Building at the intersection of<br /><strong>AI × Data × Business</strong></p></div><Portrait variant="portfolio" /><p className={`${s.meta} ${s.portfolioAside}`}>Ideas<br />Data<br />People<br />Impact</p></div>
    <div className={s.featured}>
      <div className={s.featuredCopy}><p className={s.redMeta}>01 / Featured project <span>AI × Learning</span></p><h3 className={s.display} data-reveal>Think<br />with <em>AI</em></h3><p className={s.partner}>Learning Partner</p><p className={s.featuredTagline}>Understand Deeper,<br />Not Just Faster.</p><p className={s.body}>{featuredProject.description}</p><div className={s.projectActions}><DestinationLink className={s.button} href={featuredProject.href}>Explore project <span aria-hidden="true">↗</span></DestinationLink><DestinationLink className={s.textLink} href="/projects/think-with-ai/">View case study <span aria-hidden="true">→</span></DestinationLink></div></div>
      <ProjectPreview />
    </div>
