...
export function About() {
  return <section id="about" className={`${s.section} ${s.about}`} aria-labelledby="about-title">
    ...
    <div className={s.aboutIntro}>
      ...
      <Portrait /><aside className={s.annotation}><p className={s.handwriting}>A more intelligent<br />and human future</p><span className={s.redDash} /><p className={s.meta}>People<br />Ideas<br />Data<br />Technology<br />Impact</p></aside>
    </div>
    <div className={s.career}>
      ...
      <div className={s.careerGrid}>{career.map((stage, index) => <article className={s.careerStage} key={stage.phase} data-reveal>
        ...
        <div className={s.careerDetail}><strong>{stage.institution}</strong><p>{stage.detail}</p></div>
    ...
    <div className={s.stats}>
      ...
      <div className={s.statsGrid}>{stats.map(stat => <div className={s.stat} key={stat.label}><p><strong>{stat.value}<sup>{stat.suffix}</sup></strong><span>{stat.label}</span></p><p className={s.meta}>{stat.detail}</p></div>)}<p className={s.handwriting}>More<br />to come…</p></div>
    </div>
  </section>;
}
